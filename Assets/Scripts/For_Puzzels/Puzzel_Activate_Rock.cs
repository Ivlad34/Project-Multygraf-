...
public class Puzzel_Activate_Rock : MonoBehaviour
{
    ...
    BoxCollider2D Colid; //Collider предмета, который нужно удалить
    CapsuleCollider2D ColidPlayer; //Collider игрока

    void OnTriggerStay2D(Collider2D other)
    {
        if(other.GetComponent<Player2D>() != null && Input.GetKey(KeyCode.E) == true) //Проверка, находится ли игрок в Collider нужного объекта и нажал ли игрок нужную клавишу
        {
            ...
            {
                Colid = del[0].GetComponent<BoxCollider2D>(); // Присваивание переменной, нужный BoxCollider2D
                Colid.enabled = false; // Отключение BoxCollider объекта
    ...
    void Update()
    {
        ...
        {
            if (Exit_from_puzzel_Rock.Exit() == true ) // Паззл собран
            {
                //for (int i = 0; i < del.Length - 1; i++) // Цикл, для отключения BoxCollider объектов
                //{
                //    Colid = del[i].GetComponent<BoxCollider2D>(); // Присваивание переменной, нужный BoxCollider
